...
class Timer extends Component {
    componentWillUnmount() {
        ...
        this.props.selectInfo(timerInfo.load.text, timerInfo.load.color);
    }
    ...
    handleStartTimer = () => {
        ...
        if (this.props.hour > 0 || this.props.min > 0 || this.props.sec > 0) {
            ...
            this.props.selectInfo(timerInfo.start.text, timerInfo.start.color);
            this.props.timeRunning(true);
            this.updateTime();
            this.timer = setInterval(this.updateTime, 1000);
        } else {
            this.props.selectInfo(timerInfo.error.text, timerInfo.error.color);
        }
    };
    ...
    handleStopTimer = () => {
        ...
        this.props.timeRunning(false);
        if (this.props.hour > 0 || this.props.min > 0 || this.props.sec > 0) {
            this.props.selectInfo(timerInfo.stop.text, timerInfo.stop.color);
        }
    };
    ...
    handlePauseTimer = () => {
        ...
        this.props.timeRunning(false);
        if (this.props.hour > 0 || this.props.min > 0 || this.props.sec > 0) {
            this.props.selectInfo(timerInfo.pause.text, timerInfo.pause.color);
        }
    };
...
const mapDispatchToProps = (dispatch) => {
    return {
        ...
        selectInfo: (text, color) =>
            dispatch(actionCreators.timerSetInfo(text, color)),
    };
};
